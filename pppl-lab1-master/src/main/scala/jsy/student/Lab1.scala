...
object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
   ...
   *
   * Partner: <Wu Yi>
   * Collaborators: <Nick Lopinski, Sabrina Kavesh>
   */
  ...
  def repeat(s: String, n: Int): String = {
    require(n >= 0)
    n match {
      case 0 => "" //matching 0 will return nothing
      case 1 => s //matching 1 will return just the input
      case _ => s + repeat(s, n - 1) //matching n will repeat the 's' calling itself until it hits base case
    }
  }
  ...
  def sqrtN(c: Double, x0: Double, n: Int): Double = { //c is number to sqrt, x0 is guess, n is amount of approx.
    require(n>=0)
    n match{
      case 0 => x0 //base case
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
    ...
    epsilon match {
      ...
      case _ => sqrtErr(c, sqrtStep(c, x0), epsilon) //uses recursion until it reaches epsilon approximation instead of n approximations
    }
  }
